class Movie:
    ...
    def watch(self):
        ...
        self.watched = True
        # self.__watched_movies.append(self.name) - - OPTION 1
        Movie.__watched_movies += 1

    def __repr__(self):
        # return f"Movie name: {self.name}; Movie director: {self.director}. Total watched " \
        #        f"movies: {len(self.__watched_movies)}" - OPTION 1
        return f"Movie name: {self.name}; Movie director: {self.director}. Total watched " \
                   f"movies: {Movie.__watched_movies}"
